...
from goals.models import Goal, GoalCategory, Role
...
class Command(BaseCommand):
    def __init__(self, *args, **kwargs):
        ...
        self.tg_client = TgClient(production.BOT_TOKEN)
    ...
    def handle_goals_list(self, msg: Message, tg_user: TgUser):
        ...
        if resp_goals:
            ...
            self.tg_client.send_message(msg.chat.id, '[you have not goals]')
    ...
    def handle_goal_categories_list(self, msg: Message, tg_user: TgUser):
        ...
        if resp_categories:
            ...
            self.tg_client.send_message(msg.chat.id, '[you have not categories]')
    ...
    def handle_save_selected_category(self, msg: Message, tg_user: TgUser):
        if msg.text.isdigit():
            ...
            if GoalCategory.objects.filter(
                    board__participants__user_id=tg_user.user_id,
                    board__participants__role__in=[Role.owner, Role.writer],
                    is_deleted=False,
                    id=cat_id
            ).exists():
    ...
    def handle_save_new_cat(self, msg: Message, tg_user: TgUser):
        ...
        if goal.is_completed:
            ...
            self.tg_client.send_message(msg.chat.id, '[something happen wrong]')
    ...
    def handle_verified_user(self, msg: Message, tg_user: TgUser):
        if msg.text == '/goals':
            ...
            match state:
                ...
                case _:
                    loggger.warning('Invalid state: %s', state)
